...
class BasicKey {
  ...
  onClick(textArea, upperChar) {
    ...
    let currentChar = localStorage['keyboardLang'] ===
    'eng' ? this.usChar : this.ruChar;
    currentChar = upperChar ? currentChar.toUpperCase() :
    currentChar.toLowerCase();
    textArea.value =
    textArea.value.slice(0, textArea.selectionStart) +
    currentChar + textArea.value.slice(textArea.selectionEnd);
    position++;
...
const capsFunction = function() {
  const capsButton = document.querySelector('.caps-button');
...
const backSpaceFunction = function(textArea) {
...
const keys =
[new BasicKey('`', 'ё', 'button'), new BasicKey('1', '1', 'button'),
  new BasicKey('2', '2', 'button'),
  new BasicKey('3', '3', 'button'),
  new BasicKey('4', '4', 'button'),
  new BasicKey('5', '5', 'button'),
  new BasicKey('6', '6', 'button'),
  new BasicKey('7', '7', 'button'),
  new BasicKey('8', '8', 'button'),
  new BasicKey('9', '9', 'button'),
  new BasicKey('0', '0', 'button'),
  new BasicKey('-', '-', 'button'),
  new BasicKey('=', '=', 'button'),
  new BackspaceKey('Back', 'Back', 'function-button'),
  new TabKey('Tab', 'Tab', 'tab-button'),
  new BasicKey('q', 'й', 'button', 'KeyQ'),
  new BasicKey('w', 'ц', 'button'),
  new BasicKey('e', 'у', 'button'),
  new BasicKey('r', 'к', 'button'),
  new BasicKey('t', 'е', 'button'),
  new BasicKey('y', 'н', 'button'),
  new BasicKey('u', 'г', 'button'),
  new BasicKey('i', 'ш', 'button'),
  new BasicKey('o', 'щ', 'button'),
  new BasicKey('p', 'з', 'button'),
  new BasicKey('[', 'х', 'button'),
  new BasicKey(']', 'ъ', 'button'),
  new BasicKey('\\', '\\', 'button'),
  new DelKey('Del', 'Del', 'dell-button'),
  new CapsKey('Caps', 'Caps', 'caps-button'),
  new BasicKey('a', 'ф', 'button'),
  new BasicKey('s', 'ы', 'button'),
  new BasicKey('d', 'в', 'button'),
  new BasicKey('f', 'а', 'button'),
  new BasicKey('g', 'п', 'button'),
  new BasicKey('h', 'р', 'button'),
  new BasicKey('j', 'о', 'button'),
  new BasicKey('k', 'л', 'button'),
  new BasicKey('l', 'д', 'button'),
  ...
  new BasicKey('v', 'м', 'button'),
  new BasicKey('b', 'и', 'button'),
  new BasicKey('n', 'т', 'button'),
  new BasicKey('m', 'ь', 'button'),
  new BasicKey(',', 'б', 'button'),
...
document.addEventListener('keydown', (event) => {
  ...
  for (let i = 0; i < arrKey.length; i++) {
    ...
    } else if (event.code === arrKey[i].getAttribute('data') && event.code ==
    'ControlLeft' && arrKey[i].className == 'ctrl-left-button') {
...
document.addEventListener('keyup', (event) => {
  const arrKey = document.querySelectorAll('[class$="button active"]');
  for (let i = 0; i < arrKey.length; i++) {
    if (event.code === `Key${arrKey[i].getAttribute('data')}`) {
...
for (let i = 0; i < keys.length; i++) {
  ...
  button.innerHTML = localStorage.getItem('keyboardLang') ===
  'eng' ?
  (`${keys[i].usChar}`) : (`${keys[i].ruChar}`);
  keyboard.append(button);
  button.addEventListener('click', () =>
    keys[i].onClick(textArea, upperChar));
  if (keys[i].usChar === 'Tab') {
    ...
    button.setAttribute('data', 'BracketLeft');
  } else if (keys[i].usChar === ']') {
    button.setAttribute('data', 'BracketRight');
  } else if (keys[i].usChar === '\\') {
